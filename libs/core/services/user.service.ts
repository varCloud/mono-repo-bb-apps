...
import { CategoryByUserModel, SaveCategoriesRequest } from '../../shared/models/categories';
...
export class UserService {
  ...
  public async updatePushTokenIfSessionActive(): Promise<void> {
    ...
    if (user && user.userId) {
      ...
      this.updateUser(user.userId, payload).pipe(take(1)).subscribe({
        next: () => {
          this.logger.info('Token push actualizado correctamente para usuario', { userId: user.userId , pushNotificationToken: payload.pushNotificationToken });
        },
        error: (error) => {
